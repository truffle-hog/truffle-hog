 ...
 * @author Jan Hermes - adjusted for concurrency
 * @param <V> the vertex type
 * @param <E> the edge type
 */
abstract public class ConcurrentAbstractLayout<V, E> implements Layout<V,E> {

    /**
     * a set of vertices that should not move in relation to the
     * other vertices
     */
    private Set<V> dontmove = ConcurrentHashMap.newKeySet();
    ...
    protected Map<V, Point2D> locations =
            LazyMap.decorate(new ConcurrentHashMap<V, Point2D>(),
     ...
     */
    protected ConcurrentAbstractLayout(Graph<V, E> graph) {
        if (graph == null)
        {
    ...
    protected ConcurrentAbstractLayout(Graph<V,E> graph, Transformer<V,Point2D> initializer) {
        this.graph = graph;
        Transformer<V, ? extends Object> chain =
                ChainedTransformer.getInstance(initializer, CloneTransformer.getInstance());
        this.locations = LazyMap.decorate(new ConcurrentHashMap<V,Point2D>(), (Transformer<V,Point2D>)chain);
        initialized = true;
    }

    protected ConcurrentAbstractLayout(Graph<V,E> graph, Dimension size) {
        this.graph = graph;
        this.size = size;
    }

    @SuppressWarnings("unchecked")
    protected ConcurrentAbstractLayout(Graph<V,E> graph, Transformer<V,Point2D> initializer, Dimension size) {
        this.graph = graph;
        Transformer<V, ? extends Object> chain =
                ChainedTransformer.getInstance(initializer, CloneTransformer.getInstance());
        this.locations = LazyMap.decorate(new ConcurrentHashMap<V,Point2D>(), (Transformer<V,Point2D>)chain);
    ...
    public void setInitializer(Transformer<V,Point2D> initializer) {
        ...
        Transformer<V, ? extends Object> chain =
                ChainedTransformer.getInstance(initializer, CloneTransformer.getInstance());
        this.locations = LazyMap.decorate(new ConcurrentHashMap<>(), (Transformer<V, Point2D>)chain);
        initialized = true;
    }
    ...
    public void setLocation(V picked, Point2D p) {
        ...
        if (p == null) {
            System.out.println("cannot set NULL as location: line 217 in ConcurrentAbstractLayout.java: setLocation()");
            return;
        }
