...
public class FXEdgeShape  {
     ...
     */
    public static class Line extends AbstractEdgeShapeTransformer<INode, edu.kit.trufflehog.model.network.graph.IConnection> {
         ...
         */
        @SuppressWarnings("unchecked")
		public Shape transform(Context<Graph<INode , edu.kit.trufflehog.model.network.graph.IConnection>, edu.kit.trufflehog.model.network.graph.IConnection> context) {
        	Graph<INode, edu.kit.trufflehog.model.network.graph.IConnection> graph = context.graph;
        	edu.kit.trufflehog.model.network.graph.IConnection IConnection = context.element;
            
            Pair<INode> endpoints = graph.getEndpoints(IConnection);
            if(endpoints != null) {
    ...
    public static class BentLine
             extends AbstractEdgeShapeTransformer<INode, edu.kit.trufflehog.model.network.graph.IConnection> implements IndexedRendering<INode, edu.kit.trufflehog.model.network.graph.IConnection> {
        ...
        protected EdgeIndexFunction<INode, edu.kit.trufflehog.model.network.graph.IConnection> parallelEdgeIndexFunction;

        @SuppressWarnings("unchecked")
		public void setEdgeIndexFunction(EdgeIndexFunction<INode, edu.kit.trufflehog.model.network.graph.IConnection> parallelEdgeIndexFunction) {
		 ...
		 */
		public EdgeIndexFunction<INode, edu.kit.trufflehog.model.network.graph.IConnection> getEdgeIndexFunction() {
		...
		public Shape transform(Context<Graph<INode, edu.kit.trufflehog.model.network.graph.IConnection>, edu.kit.trufflehog.model.network.graph.IConnection> context) {
            ...
            if(parallelEdgeIndexFunction != null) {
                index = parallelEdgeIndexFunction.getIndex(graph, IConnection);
            }
    ...
    public static class QuadCurve
           extends AbstractEdgeShapeTransformer<INode, IConnection> implements IndexedRendering<INode, IConnection> {
        ...
        protected EdgeIndexFunction<INode, IConnection> parallelEdgeIndexFunction;

        @SuppressWarnings("unchecked")
		public void setEdgeIndexFunction(EdgeIndexFunction<INode, IConnection> parallelEdgeIndexFunction) {
            this.parallelEdgeIndexFunction = parallelEdgeIndexFunction;
            loop.setEdgeIndexFunction(parallelEdgeIndexFunction);
        }
       ...
       /**
		 * @return the parallelEdgeIndexFunction
		 */
		public EdgeIndexFunction<INode, IConnection> getEdgeIndexFunction() {
			return parallelEdgeIndexFunction;
		}
	...
	/**
         ...
         */
        @SuppressWarnings("unchecked")
		public Shape transform(Context<Graph<INode, IConnection>, IConnection> context) {
        	final Graph<INode, IConnection> graph = context.graph;
        	final IConnection connection = context.element;

            if (connection.getDest().getAddress().isMulticast()) {

                final IRenderer rendererComp = connection.getComponent(ViewComponent.class).getRenderer();
                return rendererComp.getShape();
            }

            Pair<INode> endpoints = graph.getEndpoints(connection);
            if(endpoints != null) {
            	boolean isLoop = endpoints.getFirst().equals(endpoints.getSecond());
            	if (isLoop) {
            		return loop.transform(context);
            ...
            int index = 1;
            if(parallelEdgeIndexFunction != null) {
                index = parallelEdgeIndexFunction.getIndex(graph, connection);
            }
